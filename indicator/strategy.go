...
func HigherLower(myData *data.LayoutData, period float64, tf string, jpy bool) {
	var start = time.Now()
	...
	var timerToOpenPosition uint16
	for i := range newPrice {
		...
		var highTemp, _ = mystats.MaxMin(newHigh[i-int(period) : i])
		var _, lowTemp = mystats.MaxMin(newLow[i-int(period) : i])
		// if highTemp > highestPrice {
		highestPrice = highTemp
		// }
		// if lowTemp < lowestPrice {
		lowestPrice = lowTemp
		// }
		testMax[i] = highTemp
		testMin[i] = lowTemp
		if newPrice[i] < lowestPrice && newPrice[i-1] > lowestPrice && timerToOpenPosition == 0 {
			sell[i] = true
			sizeSell++
			timerToOpenPosition = 5
		} else {
			sell[i] = false
		}
		if newPrice[i] > highestPrice && newPrice[i-1] < highestPrice && timerToOpenPosition == 0 {
			buy[i] = true
			sizeBuy++
			timerToOpenPosition = 5
		} else {
