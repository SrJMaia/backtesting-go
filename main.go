...
func main() {
	...
	*/
	...
	for period := 2; period < 52; period += 2 {
		...
		indicator.HigherLower(&myData, float64(period), tf, jpy)
		if myData.SizeBuy < 2 || myData.SizeSell < 2 {
			continue
		}
		backtest.Optimize(&myData, &myAnalysisStruct, "netting", capital, jpy, saveCsvBacktest, leverage, tf, float64(period))
		var optimizationElapsed = time.Since(optimizationTime)
		table.LoopStepsPrint(optimizationElapsed, "outside", float64(period))
	}
